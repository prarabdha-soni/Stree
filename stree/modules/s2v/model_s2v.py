...
from .audio_utils import AudioInjector_Stree, CausalAudioEncoder
...
class StreeS2VSelfAttention(StreeSelfAttention):
...
class StreeS2VAttentionBlock(StreeAttentionBlock):
    ...
    def __init__(self,
        ...
        super().__init__(dim, ffn_dim, num_heads, window_size, qk_norm,
                         cross_attn_norm, eps)
        self.self_attn = StreeS2VSelfAttention(dim, num_heads, window_size,
                                             qk_norm, eps)
...
class StreeModel_S2V(ModelMixin, ConfigMixin):
    ignore_for_config = [
        'args', 'kwargs', 'patch_size', 'cross_attn_norm', 'qk_norm',
        'text_dim', 'window_size'
    ]
    _no_split_modules = ['StreeS2VAttentionBlock']
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            StreeS2VAttentionBlock(dim, ffn_dim, num_heads, window_size, qk_norm,
                                 cross_attn_norm, eps)
        ...
        all_modules, all_modules_names = torch_dfs(
            self.blocks, parent_name="root.transformer_blocks")
                    self.audio_injector = AudioInjector_Stree(
            all_modules,
            all_modules_names,
            dim=self.dim,
